...
{
  ros::init(argc, argv, "sbf_tcp_cpu");
  ...
  // Read parameters
  node_handle_.param("/sbf_tcp/address", address, std::string("127.0.0.1"));
  node_handle_.param("/sbf_tcp/port", port, 28004);
  node_handle_.param("/sbf_tcp/sbf_topic", sbf_topic, std::string("rtklib_nav"));
  node_handle_.param("/sbf_tcp/frame_id", frame_id, std::string("gps"));
  node_handle_.param("/sbf_tcp/rate", rate, 0.0);
